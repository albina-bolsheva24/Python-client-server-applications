...
def main():
    ...
    SERVER_LOGGER.info(f'Запущен сервер, порт для подключений: {listen_port},'
                       ...
                       f'Если адрес не указан, принимаются соединения любых адресов.')
    ...
    while True:
        ...
        try:
            ...
            SERVER_LOGGER.error(f'Не удалось декодировать Json строку, полученную от'
                                f'клиента {client_address}. Соединения закрываются.')
            client.close()
        except IncorrectDataRecivedError:
            SERVER_LOGGER.error(f'От клиента {client_address} принятые некоторые данные.'
                                f'Соединения закрываются.')
            client.close()
